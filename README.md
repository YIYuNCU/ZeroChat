...
---
## 已知BUG：
- 受个人能力影响，目前记忆功能仅在单聊场景下经过测试，群聊场景可能存在问题（如记忆错乱、记忆丢失等），后续会进行适配和修复
...
- 将记忆截取方式从固定抛弃最旧消息改为滑动窗口（以适配deepseek的缓存功能，降低开销），并引入以下机制
  ...
  - 衔接事件生成助手（需要手动配置data/roles/1000000000002/profile.json中的API_KEY）：当你在在对话的事件并未完成时突然有其他事要忙，下次对话时会在两次对话中插入合理的过度事件(当前默认关闭)
- 为女性角色添加“月经事件”以增强真实性（需要手动修改对应角色的profile，将“gender”修改为“women”以开启功能）
- 允许在前端增减AI表情和用户表情，并自动适配新增的分类
## 未来技术修改方向：
- [ ]修复记忆功能对群聊的支持                                                            
