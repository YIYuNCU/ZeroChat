...
import '../core/refresh_queue.dart';
...
class RealtimeSyncService {
  ...
  static Future<void>? _resyncFuture;
  static RefreshQueue? _chatQueue;
  static RefreshQueue? _taskQueue;
  static RefreshQueue? _momentQueue;
  ...
  static void init() {
    ...
    _initialized = true;
    _chatQueue = RefreshQueue(() async {
      final resync = _resyncFuture;
      if (resync != null) await resync;
      await MessageStore.instance.syncFromBackendSnapshot();
    }, minGap: _chatSyncMinGap);
    _taskQueue = RefreshQueue(() async {
      final resync = _resyncFuture;
      if (resync != null) await resync;
      await TaskService.fetchFromBackend(force: true);
    }, minGap: _taskSyncMinGap);
    _momentQueue = RefreshQueue(() async {
      final resync = _resyncFuture;
      if (resync != null) await resync;
      await MomentsService.instance.fetchFromBackend(force: true);
    }, minGap: _momentSyncMinGap);
    ...
    _subscription = SecureWebSocketClient.instance.serverPushStream.listen(
      (event) async {
        ...
        if (_isChatPush(type)) {
          unawaited(_refreshSafely(_chatQueue!));
        }
        ...
        if (_isTaskPush(type)) {
          unawaited(_refreshSafely(_taskQueue!));
        }

        if (_isMomentPush(type)) {
          unawaited(_refreshSafely(_momentQueue!));
        }
      },
    ...
    _reconnectSubscription = SecureWebSocketClient.instance.onReconnectedStream
        .listen(
          (_) => unawaited(
            resyncAll().catchError((Object error) {
              debugPrint(
                'RealtimeSyncService: reconnect resync failed: $error',
              );
            }),
          ),
          onError: (Object error, StackTrace stackTrace) {
  ...
  static Future<void> _refreshSafely(RefreshQueue queue) async {
    try {
      await queue.request();
    } catch (e) {
      debugPrint('RealtimeSyncService: refresh failed: $e');
    }
  }

  static Future<void> resyncAll() {
    return _resyncFuture ??= Future<void>.microtask(_resyncAll).whenComplete(
      () {
        _resyncFuture = null;
      },
    );
  ...
  static Future<void> _resyncAll() async {
    ...
    await Future.wait([
      _resyncChats(),
      TaskService.fetchFromBackend(force: true),
      MomentsService.instance.syncIfHashMismatch(force: true),
    ]);
  }

  static Future<void> _resyncChats() async {
    try {
      ...
      if (proactiveMigrationComplete) {
        ...
        throw StateError('Proactive configuration migration is incomplete');
      }
      ...
      await ChatController.instance.startPushRecovery();
